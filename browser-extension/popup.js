...
function extractCurrentPage() {
  ...
  const textRoot = clone.querySelector("article, main, [role='main']") || clone.body || clone.documentElement;
  const text = textContent(textRoot?.innerText || textRoot?.textContent || description).slice(0, 50000);
  ...
  return {
    ...
    description,
    text,
    imageCandidates: visibleImageUrls().slice(0, 12),
  };
}
